// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ObjectTypeLockedDecorator.cs" company="Allors bvba">
...
namespace Allors.R1.Development.Repository.WinForms.Decorators
{
    ...
    public class ObjectTypeLockedDecorator : SuperclassTypeEditor.ISource, SuperinterfacesTypeEditor.ISource
    {
        private readonly ObjectType type;

        public ObjectTypeLockedDecorator(ObjectType type)
        {
            this.type = type;
        }
        ...
        public ObjectType[] PossibleSuperClasses
        {
            get
            {
                ...
                foreach (var domainType in Domain.GetDomain(this.type).CompositeObjectTypes)
                {
                    if (!domainType.IsInterface && domainType.IsAbstract && !domainType.Equals(this.type))
                    {
                        superClasses.Add(domainType);
                    }
                }
        ...
        public ObjectType[] PossibleSuperinterfaces
        {
            get
            {
                ...
                foreach (var objectType in Domain.GetDomain(this.type).CompositeObjectTypes)
                {
                    if (objectType.IsInterface)
                    {
                        superinterfaces.Add(objectType);
                    }
                }
